...
vector<int> books;
int B, L, D;
multimap<int, Library> librariesScoreMap; //float - scorul bibliotecii
...
int main() {
    ...
    for(int i = 0; i < L ; i++){
        ...
        //score = score / days;
        librariesScoreMap.insert(pair<int,Library> (lib.signupD, lib));
        //librariesScoreMap[lib.signupD] = lib;
    }
    ...
    Dcopy = D;
    int index2 = signedUpLibraries;
    for(auto it = librariesScoreMap.begin(); it != librariesScoreMap.end(); ++it){
        index2--;
        Library lib = it->second; //value
        out<<lib.index<<" ";
